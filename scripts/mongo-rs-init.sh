...
docker exec -it \
  "$CONTAINER_ID" \
  bash -c "echo '${RSCONFIG}' | mongo --ssl --sslAllowInvalidHostnames --sslCAFile /run/secrets/mongodb_ca_crt --sslPEMKeyFile /run/secrets/mongodb_cert_pem -u root -p $PASSWORD"
